...
impl PodPorts {
    ...
    fn collect_port(&self, port_match: &polixy::server::Port) -> Option<Vec<u16>> {
        match port_match {
            polixy::server::Port::Number(ref port) => Some(vec![*port]),
            polixy::server::Port::Name(ref name) => self.by_name.get(name).cloned(),
        }
    }
}
